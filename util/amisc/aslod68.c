...
void emits(struct outsect* section, struct outsect* nextsect)
{
	...
	if (nextsect)
		real_size = align(section->os_base + real_size, nextsect->os_lign) - section->os_base;
...
int rhead(FILE* f, struct outhead* head)
{
	...
	c = buf;
	head->oh_magic = uget2(c);
	c += 2;
	head->oh_stamp = uget2(c);
	c += 2;
	head->oh_flags = uget2(c);
	c += 2;
	head->oh_nsect = uget2(c);
	c += 2;
	head->oh_nrelo = uget2(c);
	c += 2;
	head->oh_nname = uget2(c);
	c += 2;
	head->oh_nemit = get4(c);
	c += 4;
...
int main(int argc, char* argv[])
{
	...
	if (!((outhead.oh_nsect == NUM_SEGMENTS) || (outhead.oh_nsect == (NUM_SEGMENTS + 1))))
		fatal("the input file must have %d sections, not %ld.", NUM_SEGMENTS, outhead.oh_nsect);
